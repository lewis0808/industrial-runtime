...
class Config {
  public:
    ...
    template <typename T> [[nodiscard]] T get(std::string_view dottedKey, T defaultValue) const {
    ...
    template <typename T> [[nodiscard]] std::optional<T> tryGet(std::string_view dottedKey) const {
        std::shared_lock lock(mutex_);
        const nlohmann::json *node = resolve(dottedKey);
        if (node == nullptr) {
            return std::nullopt;
