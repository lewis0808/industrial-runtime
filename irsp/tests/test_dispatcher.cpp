...
const std::string *errCode(const IrspValue &v) {
    const auto *e = std::get_if<IrspError>(&v);
...
int main() {
    ...
    // PING。
    IR_CHECK(std::get<IrspSimple>(disp.handle(s, cmd({"PING"}))).text == "PONG");
    IR_CHECK(std::get<IrspBulk>(disp.handle(s, cmd({"PING", "hi"}))).data == "hi");
    ...
    IR_CHECK(std::holds_alternative<IrspNull>(disp.handle(s, cmd({"GET", "no/such"}))));

    // GET 参数错误。
    IR_CHECK(*errCode(disp.handle(s, cmd({"GET"}))) == "WRONG_ARITY");

    // EXISTS。
    IR_CHECK(std::get<IrspInteger>(disp.handle(s, cmd({"EXISTS", "factory1/line1/temp"}))).value ==
             1);
    IR_CHECK(std::get<IrspInteger>(disp.handle(s, cmd({"EXISTS", "x"}))).value == 0);

    // MGET。
    {
        auto r = disp.handle(s, cmd({"MGET", "factory1/line1/temp", "no/such"}));
        const auto &a = std::get<IrspArray>(r);
        IR_CHECK_EQ(a.items.size(), std::size_t{2});
        IR_CHECK(std::holds_alternative<IrspMap>(a.items[0]));
        IR_CHECK(std::holds_alternative<IrspNull>(a.items[1]));
    }
    ...
    // SCAN。
    {
        ...
        const auto &a = std::get<IrspArray>(r);
        IR_CHECK(std::get<IrspBulk>(a.items[0]).data == "0"); // nextCursor
        IR_CHECK_EQ(std::get<IrspArray>(a.items[1]).items.size(), std::size_t{2});
    }

    // WATCH / SUBSCRIBE 与路由。
    {
        auto r = disp.handle(s, cmd({"WATCH", "factory1/line1/temp"}));
        IR_CHECK(std::get<IrspInteger>(r).value == 1);
    ...
    IR_CHECK(*errCode(disp.handle(s, cmd({"WATCH", "factory1/+/temp"}))) == "ERR"); // 非具体
    {
        ...
        IR_CHECK(std::get<IrspInteger>(r).value == 2); // 已含 WATCH 的 1 个
    ...
    // SET 写回。
    {
        ...
        IR_CHECK(std::holds_alternative<IrspSimple>(r));
        IR_CHECK(std::get<IrspSimple>(r).text == "OK");
